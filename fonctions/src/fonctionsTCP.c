...
*/
int socketClient(char* nomMach, ushort noPort) {
    ...
    */
    ...
    char noPortChar[12];
    sprintf(noPortChar, "%d", noPort);

    // récupération de la liste des adresses corespondante au serveur
    err = getaddrinfo(nomMach, noPortChar, &hints, &result);
    if (err != 0) {
...
* retourne :  socket connexion (int)
* 
* utiliser sur la socket:  int accept(int sockfd, NULL, NULL)
*/
int socketServeur(ushort noPort) {
    ...
    */
    ...
    addServ.sin_port = htons(noPort); // conversion en format réseau (big endian)
